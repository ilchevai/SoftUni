...
class ConcertTrackerApp:
    ...
    def create_musician(self, musician_type: str, name: str, age: int):
        ...
        if name in self.MUSICIANS_NAMES:
            raise Exception(f"{name} is already a musician!")
        if musician_type not in self.VALID_MUSICIAN_TYPE:
            raise ValueError("Invalid musician type!")

        musician = self.VALID_MUSICIAN_TYPE[musician_type](name, age)
        self.musicians.append(musician)
        self.MUSICIANS_NAMES.append(name)
        return f"{name} is now a {musician_type}."

    def create_band(self, name):
        if name in self.BANDS_NAMES:
            raise ValueError(f"{name} band is already created!")

        band = Band(name)
        self.bands.append(band)
        self.BANDS_NAMES.append(name)
